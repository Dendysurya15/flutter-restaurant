...
class DashboardOwnerView extends GetView<DashboardOwnerController> {
  ...
  Widget _buildOrderCard(dynamic order) {
    return Obx(() {
      // Check if this order is newly received
      final isNewOrder = controller.newOrderIds.contains(order.id);
      final borderColor = isNewOrder
          ? Colors.green
      ...
      return Card(
        ...
        shape: RoundedRectangleBorder(
          ...
          side: BorderSide(color: borderColor, width: isNewOrder ? 3 : 1),
        ),
        child: InkWell(
      ...
      AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              Get.back();
              controller.acceptOrder(order.id, 15);
            },
          ...
          TextButton(
            onPressed: () {
              Get.back();
              controller.acceptOrder(order.id, 30);
            },
          ...
          TextButton(
            onPressed: () {
              Get.back();
              controller.acceptOrder(order.id, 45);
            },
